...
def reader_GNSS(filename):
    ...
    for i in range(len(content) - start_num):  # 遍历数据块
        ...
        if line[0] == 'C':
            for j in range(0, 8):
                ...
                if line_index < len(content) and line_index not in processed_lines:
    ...
    n_data_lines_nums = int((len(nfile_lines) - start_num) / 8) # 计算数据块的个数
    ...
    for j in range(n_data_lines_nums):
        n_dic = {}
        for i in range(8):
            data_content = nfile_lines[start_num + 8 * j + i] # 第j组，第i行
            n_dic['数据组数'] = j + 1
            if i == 0:
            ...
            if i == 5:
                n_dic['IDOT'] = float((data_content.strip('\n')[4:23]))
    ...
    with open('北斗卫星.csv', 'w', newline='') as f:
        header = ['数据组数', '卫星PRN号', '历元', '卫星钟偏差(s)', '卫星钟漂移(s/s)', '卫星钟漂移速度(s/s*s)', 'IODE','C_rs', 'n', 'M0', 'C_uc', 'e', 'C_us', 'sqrt_A', 'TEO', 'C_ic', 'OMEGA', 'C_is', 'I_0', 'C_rc', 'w','OMEGA_DOT', 'IDOT', 'L2_code', 'PS_week_num', 'L2_P_code', '卫星精度(m)', '卫星健康状态', 'TGD','IODC','X', 'Y', 'Z']
